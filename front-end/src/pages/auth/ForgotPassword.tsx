...
import { API_BASE_URL, REQUEST_PASSWORD_RESET_ENDPOINT } from '@/config/auth';
...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}${REQUEST_PASSWORD_RESET_ENDPOINT}`, {
        method: 'POST',
        headers: {
          ...
          >
            <div className="bg-card rounded-2xl shadow-medium border border-border/50 p-8">
              ...
              <p className="text-muted-foreground mb-8">
                Si l'adresse email <span className="font-semibold text-foreground">{email}</span> existe dans notre système,
